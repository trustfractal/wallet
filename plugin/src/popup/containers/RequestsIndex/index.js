...
import {
  ...
  getDeclinedRequests,
  getPendingRequests,
} from "@redux/stores/user/reducers/requests/selectors";
...
function RequestsIndex() {
  const accepted = useUserSelector(getAcceptedRequests);
  const declined = useUserSelector(getDeclinedRequests);
  const pending = useUserSelector(getPendingRequests);
