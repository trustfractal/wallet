...
export class MintingRegistrar {
  constructor(private readonly storage: Storage, private readonly statusCheckSleepSeconds: number) {}

  async maybeTryRegister() {
    const lastCheck = await this.storage.getItem('minting_registrar/last_check');
    const now = new Date().getTime() / 1000;
    const shouldCheck = lastCheck == null || now > parseInt(lastCheck) + this.statusCheckSleepSeconds;
    if (!shouldCheck) return;

    const protocol = await ProtocolService.fromStorage(this.storage);
    const isRegistered = await protocol.isRegisteredForMinting(protocol.address());
    if (isRegistered) {
      ...
      console.log(`Successfully registered for minting ${'foo'}`);
    }
